      ...
      reader.onload = (e) => {
        try {
          ...
          const items = jsonData.map(row => ({
            ...
            MedicalCategory: row['病案费用大类'] || row['MedicalCategory'] || '',
            MedicalSubCategory: row['病案费用小类'] || row['MedicalSubCategory'] || '',
            ProvincePrice: parseFloat(row['省单价'] ?? row['省标价'] ?? row['ProvincePrice']) || 0,
            CityPrice: parseFloat(row['市单价'] ?? row['市标价'] ?? row['CityPrice']) || 0,
            CountyPrice: parseFloat(row['县单价'] ?? row['县标价'] ?? row['CountyPrice']) || 0,
            Price: parseFloat(row['单价'] ?? row['价格'] ?? row['Price']) || 0,
